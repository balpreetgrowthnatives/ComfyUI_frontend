...
`/cloud/supported-nodes` (and `/zh-CN/`) lists custom-node packs preinstalled on Comfy Cloud, joined with public metadata from the [ComfyUI Custom Node Registry](https://registry.comfy.org) ([`api.comfy.org`](https://api.comfy.org)). See [`docs/cloud-supported-nodes.md`](docs/cloud-supported-nodes.md) for the build pipeline, source-file map, and key invariants.
